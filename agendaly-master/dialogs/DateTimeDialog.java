...
public class DateTimeDialog {
    ...
    public static void editDate(Context context,
                                TextView dateDisplay, int[] date) {
        final Calendar c = Calendar.getInstance();
        date[0] = c.get(Calendar.DAY_OF_MONTH);
        date[1] = c.get(Calendar.MONTH);
        date[2] = c.get(Calendar.YEAR);
        ContextThemeWrapper newContext = new ContextThemeWrapper(context,
                R.style.Theme_Agendaly_DatePickerDialog);
    ...
    public static void editTime(Context context,
                                TextView hourDisplay, int[] time) {
        final Calendar c = Calendar.getInstance();
        time[0] = c.get(Calendar.HOUR_OF_DAY);
        time[1] = c.get(Calendar.MINUTE);
        ContextThemeWrapper newContext = new ContextThemeWrapper(context,
                R.style.Theme_Agendaly_Dialog);
